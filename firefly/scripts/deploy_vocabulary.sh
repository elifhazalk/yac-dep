...
if [ "$#" -ne 2 ]; then
  echo "Usage: $0 {Download Target DIR} {Vocabulary Target DIR}" >&2
  exit 1
fi
...
DOWNLOAD_TARGET=$1
VOCABULARY_DEPLOY_DIR=$2

echo "DOWNLOAD_TARGET : $DOWNLOAD_TARGET"
echo "VOCABULARY_DEPLOY_DIR : $VOCABULARY_DEPLOY_DIR"


## slam_node config
VOCABULARY_ARCHIVE_NAME="vocabulary.tar.gz"
VOCABULARY_DEPLOY_TARGET=$VOCABULARY_DEPLOY_DIR
check_source_archive $DOWNLOAD_TARGET ${VOCABULARY_ARCHIVE_NAME}
check_folder_status ${VOCABULARY_DEPLOY_TARGET}

# untar archive to target
if [ "$HOSTNAME" = "firefly" ]; then
    # vocabulary deploy
    echo "extracting ${VOCABULARY_ARCHIVE_NAME}"
